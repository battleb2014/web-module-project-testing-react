...
test('renders Loading component when prop show is null', () => {
    render(
        <Show show = { null } selectedSeason = 'none' />
    );
...
test('handleSelect is called when an season is selected', () => {
    ...
    render(
        <Show show = { testShow } selectedSeason = 'none' handleSelect = { mockHandleSelect } />
    );

    const selectEpisodes = screen.queryByLabelText(/select a season/i);

    userEvent.selectOptions(selectEpisodes, ['1'])
